...
const REFRESH_MS = 5 * 60 * 1e3;
const BAR_WIDTH = 10;
const CLAUDE_USAGE_URL = "https://api.anthropic.com/api/oauth/usage";
const USAGE_FETCH_TIMEOUT_MS = 45e3;
...
const TOKEN_EXPIRY_SAFETY_MS = 5 * 60 * 1e3;
...
async function refreshUsageViews(api) {
  if (!refreshHandlers.size) {
    api.ui.toast({
      variant: "warning",
      message: "OpenCode usage sidebar is not mounted"
    });
    return;
  }
  ...
  api.ui.toast({
    variant: "success",
    message: "OpenCode usage updated"
  });
}
...
function formatResetValue(value) {
  ...
  if (typeof value !== "number" || !Number.isFinite(value)) return void 0;
  const timestamp = Math.abs(value) < 1e12 ? value * 1e3 : value;
...
function fetchFailureSnapshot(active, provider, result) {
...
function providerLabel(providerKey, fallback) {
...
function providerMessageName(providerKey, fallback) {
  if (providerKey === "unsupported") return (fallback || "provider").toLowerCase();
  return providerKey;
}
function normalizeProvider(providerID, modelID) {
...
function modelRef(message) {
  if (message.role === "user") {
    return isModelReference(message.model) ? message.model : void 0;
  }
  return {
...
function activeModelFromRef(api, ref) {
...
function activeModel(api, sessionID) {
...
function unavailableSnapshot(active, message) {
  return {
    status: "unavailable",
...
function normalizeClaudePercent(value) {
  return value >= 0 && value <= 1 ? value * 100 : value;
}
function claudeCandidateRecords(root) {
  const records = [root];
...
function claudeWindow(record, key, label) {
  const value = record[key];
  if (!isRecord(value)) return void 0;
  const utilization = readFirstNumber(value, ["utilization", "usage", "used", "percent"]);
  if (typeof utilization !== "number") return void 0;
  return {
    label,
...
function parseClaudeUsage(payload, active) {
  if (!isRecord(payload)) return void 0;
  for (const record of claudeCandidateRecords(payload)) {
    const rows = [claudeWindow(record, "five_hour", "5h"), claudeWindow(record, "seven_day", "7d"), claudeWindow(record, "seven_day_sonnet", "7d sonnet")].filter((row) => Boolean(row));
...
async function readClaudeAccessToken() {
  try {
    ...
    if (!isRecord(oauth)) return void 0;
...
async function loadClaudeUsage(active) {
...
function minimaxCandidateRecords(root) {
  const records = [root];
...
function parseMiniMaxUsage(payload, active) {
  if (!isRecord(payload)) return void 0;
  const percentKeys = ["used_percent", "usage_percent", "percent", "usedPercentage", "usagePercentage"];
  const remainingKeys = ["remaining", "remain", "remains", "remain_tokens", "remaining_tokens", "available_tokens", "unused_tokens", "left_tokens"];
  ...
  for (const record of minimaxCandidateRecords(payload)) {
    ...
    let percent = typeof directPercent === "number" ? directPercent : void 0;
    ...
    return {
      ...
      provider: active.providerKey,
      providerLabel: active.providerLabel,
      plan: quotaParts.filter(Boolean).join(" / ") || void 0,
      rows
    };
  }
  return void 0;
}
async function loadMiniMaxUsage(active) {
...
function parseOpenRouterUsage(payload, active) {
  if (!isRecord(payload)) return void 0;
  ...
  return {
    ...
    providerLabel: active.providerLabel,
    account: label,
    plan: quotaParts.join(" / ") || void 0,
    rows
  };
}
async function loadOpenRouterUsage(active) {
...
function codexWindow(record, key, label) {
  const value = record[key];
  if (!isRecord(value)) return void 0;
  const percent = readNumber(value, "used_percent");
  if (typeof percent !== "number") return void 0;
  return {
    label,
...
function parseCodexUsage(payload, active, model) {
  ...
  const rows = [codexWindow(rateLimit, "primary_window", "primary"), codexWindow(rateLimit, "secondary_window", "secondary")].filter((row) => Boolean(row));
...
function readCodexConfigModel(raw) {
...
async function loadCodexUsage(active) {
...
function geminiSelectedModel(settings) {
  ...
  if (isRecord(selected)) {
    const model2 = readFirstString(selected, ["name", "model", "modelId", "id"]);
...
function readGeminiSettingsProject(settings) {
  return settings ? readString(settings, "cloudaicompanionProject") : void 0;
}
...
function parseGeminiProject(payload) {
  if (!isRecord(payload)) return void 0;
...
async function discoverGeminiProject(token, active) {
  ...
  if (!result.ok) return {
    snapshot: fetchFailureSnapshot(active, "gemini", result)
  };
  const project = parseGeminiProject(result.payload);
  if (!project) return {
    snapshot: unavailableSnapshot(active, "usage API unavailable for gemini (project discovery failed)")
  };
  return {
    project
  };
}
function payloadBuckets(payload) {
...
function geminiBucketRow(bucket) {
  const remainingFraction = readNumber(bucket, "remainingFraction");
  if (typeof remainingFraction !== "number") return void 0;
...
function parseGeminiUsage(payload, active, selectedModel) {
  const rows = payloadBuckets(payload).filter(isRecord).map(geminiBucketRow).filter((row) => Boolean(row));
  if (!rows.length) return void 0;
  ...
  const conciseRows = orderedRows.filter((row) => Boolean(row)).slice(0, 3);
  return {
...
async function loadGeminiUsage(active) {
...
function zaiLimits(payload) {
  if (!isRecord(payload)) return [];
  const data = isRecord(payload.data) ? payload.data : payload;
  return Array.isArray(data.limits) ? data.limits : [];
}
function parseZaiUsage(payload, active) {
  const rows = [];
  for (const item of zaiLimits(payload)) {
    if (!isRecord(item)) continue;
    const type = readString(item, "type");
    const label = type === "TOKENS_LIMIT" ? "tokens" : type === "TIME_LIMIT" ? "time" : void 0;
    if (!label) continue;
    ...
    rows.push({
      label,
      used,
      reset: formatResetValue(item.nextResetTime)
    });
  }
  if (!rows.length) return void 0;
  return {
...
async function loadZaiUsage(active) {
...
function UsageBar(props) {
  ...
  return (() => {
    var _el$ = _$createElement("box"), _el$2 = _$createElement("text"), _el$3 = _$createTextNode(` [`), _el$4 = _$createElement("span"), _el$5 = _$createElement("span"), _el$6 = _$createTextNode(`] `), _el$7 = _$createElement("span"), _el$8 = _$createElement("text"), _el$9 = _$createTextNode(`reset `);
    ...
    _$insert(_el$4, () => bar().filled);
    _$insert(_el$5, () => bar().empty);
    _$insert(_el$7, () => bar().percent);
    _$insertNode(_el$8, _el$9);
    _$insert(_el$8, reset, null);
    _$effect((_p$) => {
      ...
      _v$4 !== _p$.o && (_p$.o = _$setProp(_el$7, "style", _v$4, _p$.o));
...
function compactPlanLabel(plan) {
  const label = plan?.split(" / ")[0]?.trim();
  if (!label || /^(credits|tokens|limit)\b/i.test(label)) return void 0;
  return label;
}
function usageTitle(snapshot, active) {
  const model = snapshot?.model?.trim() || (active.providerID === "unknown" ? void 0 : active.display);
  const details = [model, compactPlanLabel(snapshot?.plan)].filter((detail) => Boolean(detail));
...
function View(props) {
  ...
  return (() => {
    var _el$0 = _$createElement("box"), _el$1 = _$createElement("box"), _el$10 = _$createElement("text"), _el$11 = _$createElement("b");
    _$insertNode(_el$0, _el$1);
    _$insertNode(_el$1, _el$10);
    _$setProp(_el$1, "flexDirection", "row");
    _$setProp(_el$1, "gap", 1);
    _$insertNode(_el$10, _el$11);
    _$insert(_el$11, () => usageTitle(snapshot(), currentModel()));
    _$insert(_el$0, _$createComponent(Show, {
      ...
      get children() {
        ...
        _$insertNode(_el$12, _$createTextNode(`checking usage...`));
    ...
    _$insert(_el$0, _$createComponent(Show, {
      get when() {
        return _$memo(() => !!!loading())() && error();
      },
      get children() {
        var _el$14 = _$createElement("text");
        _$setProp(_el$14, "wrapMode", "word");
        _$insert(_el$14, error);
        _$effect((_$p) => _$setProp(_el$14, "fg", theme().error, _$p));
        return _el$14;
      }
    }), null);
    _$insert(_el$0, _$createComponent(Show, {
      get when() {
        return _$memo(() => !!(!loading() && snapshot()))() && !error();
      },
      get children() {
        ...
        _$insert(_el$15, _$createComponent(For, {
          ...
          children: (row) => _$createComponent(UsageBar, _$mergeProps({
            get api() {
              return props.api;
            }
          }, row))
        ...
        _$insert(_el$15, _$createComponent(Show, {
          ...
          get children() {
            ...
            _$insert(_el$16, () => snapshot()?.message);
            _$effect((_$p) => _$setProp(_el$16, "fg", snapshot()?.status === "unavailable" ? theme().textMuted : theme().error, _$p));
            return _el$16;
          }
        }), null);
        return _el$15;
